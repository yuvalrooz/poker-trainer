...
use poker::{apply_action, calculate_equity, calculate_heuristic_odds, new_game, to_view, EquityResult, GameState, GameView, HeuristicOdds};
use std::sync::Mutex;
use tauri::State;
...
#[tauri::command]
fn get_heuristic_odds(state: State<AppState>) -> Result<Option<HeuristicOdds>, String> {
    let lock = state.0.lock().unwrap();
    let game = lock.as_ref().ok_or("No active game")?;
    Ok(calculate_heuristic_odds(game))
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![new_hand, take_action, get_equity, get_heuristic_odds])
